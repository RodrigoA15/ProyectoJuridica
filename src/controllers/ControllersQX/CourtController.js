...
export const courtById = async (req, res) => {
  try {
    ...
    const oneCourt = await sequelize.query(
      "SELECT RUNT_ENTE, ENTES_JURIDICOS.DESC_ENTE_JURIDICO, RUNT_TIPO_ENTIDAD_JURIDICA.MUNICIPIO FROM ENTES_JURIDICOS INNER JOIN RUNT_TIPO_ENTIDAD_JURIDICA ON ENTES_JURIDICOS.ID_ENTE_JURIDICO = RUNT_TIPO_ENTIDAD_JURIDICA.ID_ENTIDAD WHERE DESC_ENTE_JURIDICO=  :DESC_ENTE_JURIDICO",
      {
